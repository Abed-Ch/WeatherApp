...
export class SnowyDirective implements OnInit, OnDestroy {
  ...
  private makeRain(): void {
    const div: HTMLDivElement = this.renderer.createElement('div');
    this.renderer.addClass(div, 'Snow');
    this.renderer.appendChild(this.element.nativeElement, div);
    var raindrop: HTMLDivElement;
    for (let i = 0; i < 100; i++) {
      raindrop = this.renderer.createElement('span');
      this.renderer.addClass(raindrop, 'snowFlakes');
      raindrop.style.setProperty('--i', i.toString());
      raindrop.style.animationDuration = Math.ceil(Math.random() * 15) + 's';
